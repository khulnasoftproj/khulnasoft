...
import (
	...
	"github.com/khulnasoftproj/khulnasoft/v2/pkg/log"
...
func (runner *Runner) setParam(c *cli.Context, commandName string, param *config.Param) error { //nolint:funlen,cyclop
	...
	param.LogColor = os.Getenv("KHULNASOFT_LOG_COLOR")
	param.KHULNASOFTVersion = runner.LDFlags.Version
	param.KhulnasoftCommitHash = runner.LDFlags.Commit
	...
	param.MaxParallelism = config.GetMaxParallelism(os.Getenv("KHULNASOFT_MAX_PARALLELISM"), logE)
	param.GlobalConfigFilePaths = finder.ParseGlobalConfigFilePaths(os.Getenv("KHULNASOFT_GLOBAL_CONFIG"))
	...
	param.PWD = wd
	param.ProgressBar = os.Getenv("KHULNASOFT_PROGRESS_BAR") == "true"
	param.Tags = parseTags(strings.Split(c.String("tags"), ","))
	param.ExcludedTags = parseTags(strings.Split(c.String("exclude-tags"), ","))

	if a := os.Getenv("KHULNASOFT_DISABLE_LAZY_INSTALL"); a != "" {
		disableLazyInstall, err := strconv.ParseBool(a)
		if err != nil {
			return fmt.Errorf("parse the environment variable KHULNASOFT_DISABLE_LAZY_INSTALL as bool: %w", err)
		}
		param.DisableLazyInstall = disableLazyInstall
	}

	if a := os.Getenv("KHULNASOFT_DISABLE_POLICY"); a != "" {
		disablePolicy, err := strconv.ParseBool(a)
		if err != nil {
			return fmt.Errorf("parse the environment variable KHULNASOFT_DISABLE_POLICY as bool: %w", err)
		}
		param.DisablePolicy = disablePolicy
	}
	if !param.DisablePolicy {
		param.PolicyConfigFilePaths = policy.ParseEnv(os.Getenv("KHULNASOFT_POLICY_CONFIG"))
	...
	if a := os.Getenv("KHULNASOFT_REQUIRE_CHECKSUM"); a != "" {
		requireChecksum, err := strconv.ParseBool(a)
		if err != nil {
			return fmt.Errorf("parse the environment variable KHULNASOFT_REQUIRE_CHECKSUM as bool: %w", err)
		}
...
func (runner *Runner) Run(ctx context.Context, args ...string) error {
	...
	app := cli.App{
		Name:           "khulnasoft",
		Usage:          "Version Manager of CLI. https://khulnasoftproj.github.io/",
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:    "log-level",
				Usage:   "log level",
				EnvVars: []string{"KHULNASOFT_LOG_LEVEL"},
			},
			&cli.StringFlag{
				Name:    "config",
				Aliases: []string{"c"},
				Usage:   "configuration file path",
				EnvVars: []string{"KHULNASOFT_CONFIG"},
			},
			&cli.StringFlag{
				Name:  "trace",
		...
		Commands: []*cli.Command{
			...
			runner.newUpdateKhulnasoftCommand(),
