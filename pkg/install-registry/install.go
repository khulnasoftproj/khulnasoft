...
type Installer interface {
	InstallRegistries(ctx context.Context, logE *logrus.Entry, cfg *khulnasoft.Config, cfgFilePath string, checksums *checksum.Checksums) (map[string]*registry.Config, error)
}
...
func (inst *MockInstaller) InstallRegistries(ctx context.Context, logE *logrus.Entry, cfg *khulnasoft.Config, cfgFilePath string, checksums *checksum.Checksums) (map[string]*registry.Config, error) {
	return inst.M, inst.Err
}

var errMaxParallelismMustBeGreaterThanZero = errors.New("MaxParallelism must be greater than zero")

func (inst *InstallerImpl) InstallRegistries(ctx context.Context, logE *logrus.Entry, cfg *khulnasoft.Config, cfgFilePath string, checksums *checksum.Checksums) (map[string]*registry.Config, error) {
	...
	for _, registry := range cfg.Registries {
		...
		wg.Add(1)
		go func(registry *khulnasoft.Registry) {
			defer wg.Done()
			if registry.Name == "" {
...
func (inst *InstallerImpl) installRegistry(ctx context.Context, logE *logrus.Entry, regist *khulnasoft.Registry, cfgFilePath string, checksums *checksum.Checksums) (*registry.Config, error) {
	...
	if regist.Type == khulnasoft.RegistryTypeLocal {
...
func (inst *InstallerImpl) getRegistry(ctx context.Context, logE *logrus.Entry, registry *khulnasoft.Registry, registryFilePath string, checksums *checksum.Checksums) (*registry.Config, error) {
	// TODO checksum verification
	// TODO download checksum file
	if registry.Type == khulnasoft.RegistryTypeGitHubContent {
...
func (inst *InstallerImpl) getGitHubContentRegistry(ctx context.Context, logE *logrus.Entry, regist *khulnasoft.Registry, registryFilePath string, checksums *checksum.Checksums) (*registry.Config, error) {
