...
func getEnabledXSysExec(osEnv osenv.OSEnv, goos string) bool {
	...
	if osEnv.Getenv("KHULNASOFT_EXPERIMENTAL_X_SYS_EXEC") == "false" {
		return false
	}
	if osEnv.Getenv("KHULNASOFT_X_SYS_EXEC") == "false" {
		return false
	}
	return true
}
...
func (ctrl *Controller) Exec(ctx context.Context, logE *logrus.Entry, param *config.Param, exeName string, args ...string) (gErr error) {
	...
	if param.DisableLazyInstall {
		if _, err := ctrl.fs.Stat(findResult.ExePath); err != nil {
			return logerr.WithFields(errExecNotFoundDisableLazyInstall, logE.WithField("doc", "https://khulnasoftproj.github.io/docs/reference/codes/006").Data) //nolint:wrapcheck
		}
	}
