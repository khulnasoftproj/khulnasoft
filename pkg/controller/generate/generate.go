...
import (
	...
	rgst "github.com/khulnasoftproj/khulnasoft/v2/pkg/install-registry"
...
func (ctrl *Controller) Generate(ctx context.Context, logE *logrus.Entry, param *config.Param, args ...string) error {
	// Find and read a configuration file (khulnasoft.yaml).
	// Install registries
	...
	//       set default value
	//   Output to Stdout or Update khulnasoft.yaml (-i)
...
func (ctrl *Controller) listPkgs(ctx context.Context, logE *logrus.Entry, param *config.Param, cfg *khulnasoft.Config, cfgFilePath string, args ...string) ([]*khulnasoft.Package, error) {
...
func (ctrl *Controller) listPkgsWithFinder(ctx context.Context, logE *logrus.Entry, param *config.Param, registryContents map[string]*registry.Config) ([]*khulnasoft.Package, error) {
	...
	arr := make([]*khulnasoft.Package, len(idxes))
...
func (ctrl *Controller) listPkgsWithoutFinder(ctx context.Context, logE *logrus.Entry, param *config.Param, registryContents map[string]*registry.Config, pkgNames ...string) ([]*khulnasoft.Package, error) {
	m := map[string]*FindingPackage{}
	ctrl.setPkgMap(logE, registryContents, m)

	outputPkgs := []*khulnasoft.Package{}
...
func (ctrl *Controller) getOutputtedPkg(ctx context.Context, logE *logrus.Entry, param *config.Param, pkg *FindingPackage) *khulnasoft.Package {
	outputPkg := &khulnasoft.Package{
